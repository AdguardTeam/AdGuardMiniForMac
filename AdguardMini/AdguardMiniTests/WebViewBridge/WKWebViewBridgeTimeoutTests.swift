...
final class WKWebViewBridgeTimeoutTests: XCTestCase {
    /// A `WebViewAsyncInvoking` mock that NEVER invokes the completion
    /// handler. Lets `DispatchQueue.main.asyncAfter` run the timer-fire
    /// path of the bridge's structured invoke wrapper.
    private final class HangingWebView: WebViewAsyncInvoking {
        ...
        func invoke(
            ...
            // Intentionally NOT calling completion — simulates a hang.
        }
    }
    ...
    func testDispatchCallback_WhenInvokeCompletes_DoesNotFireOnJavaScriptTimeout_FiresOnJavaScriptSuccess()
        async throws {
        let mock = MockWebView()  // completes immediately via Task 2 spy
