# the base idea from https://parsy.readthedocs.io/en/latest/howto/other_examples.html#proto-file-parser
# -*- coding: utf-8 -*-

# Parser for protocol buffer .proto files
import enum as stdlib_enum
from string import ascii_letters, digits, hexdigits, octdigits, punctuation, whitespace
...
_description = (letter | decimalDigit | string(" ") | string("_")).many().concat()
description = lexeme(_description).desc('description')

_multiline_description = (letter | decimalDigit | punctuation_marks | string(" ") | regex(r"\r\n") | regex(r"\n") | regex(r"\t") | string("* ") | string("'") | string("\"")).many().concat()
multiline_description = lexeme(_multiline_description).desc('multiline_description')
...
hexLit     = regex("0[xX][0-9a-fA-F]+").desc('hexLit').map(convert_hex)
intLit     = decimalLit | octalLit | hexLit

# Floating-point literals
decimals = r'[0-9]+'
exponent  = r'[eE][+-]?' + decimals
...
# String literals
hexEscape = regex(r"\\[xX]") >> regex("[0-9a-fA-F]{2}").map(convert_hex).map(chr)
...
# Apply the parsed sign to the numeric value instead of collapsing every
# constant to +1/-1, so option/default values survive with their magnitude.
# `sign` matches the empty string for unsigned values, so an unsigned number
# keeps its value unchanged.
signedNumberChange = lambda s, num: -num if s == "-" else num
sign = regex("[-+]?")
...
# Package
# `seq` of a single parser yields a 1-tuple; unwrap it so `Package.identifier`
# holds the package name string, instead of leaking a tuple downstream.
package = (lexeme("package") >> fullIdent << SEMI).map(lambda name: Package(name))
...
field = seq(is_present("repeated").tag('repeated'),
            ...
            # A trailing `//` comment is optional and must be single-line;
            # it must not swallow the following field declaration.
            (SLASHES >> regex(r'[^\r\n]*').desc('comment')).optional().tag('description'),
            ).combine_dict(Field)
...
describedRpc = seq(COMMENT_IN >> multiline_description.tag('description') << COMMENT_OUT,
                rpc.tag('body'),
                ).combine_dict(DescribedObject)

service = seq(lexeme("service") >> serviceName.tag('name'),
              LBRACE
              >> (option | describedRpc | emptyStatement).many().map(exclude_none).tag('body')
              << RBRACE
              ).combine_dict(Service)


# Proto file
topLevelDef = seq(COMMENT_IN >> multiline_description.tag('description') << COMMENT_OUT,
                (message | enum | service).tag('body'),
                ).combine_dict(DescribedObject)
