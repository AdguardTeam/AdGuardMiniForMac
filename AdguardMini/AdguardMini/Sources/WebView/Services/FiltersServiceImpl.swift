...
final class FiltersServiceImpl: FiltersService.ServiceType {
    ...
    func updateFilters(_ message: FiltersUpdate, _ promise: @escaping (OptionalError) -> Void) {
        LogInfo("DIAG: updateFilters START count=\(message.ids.count) enabled=\(message.isEnabled)")
        Task {
            LogInfo("DIAG: updateFilters before setFilters")
            await self.filtersSupervisor.setFilters(message.ids.map(Int.init), enabled: message.isEnabled)
            LogInfo("DIAG: updateFilters after setFilters -> resolve")
    ...
    func checkCustomFilter(_ message: Path, _ promise: @escaping (FilterOrError) -> Void) {
        ...
        Task {
            ...
            // Compromised page cannot probe arbitrary files (mirrors
            // `UserRulesServiceImpl.importUserRules` + `showInFinder`).
            if let confinementPath = PathConfiner.confinementPath(for: message.path) {
                guard PathConfiner.isConfinable(
                    ...
                    LogError("checkCustomFilter rejected: path not confined")
                    filter.error = true
                    promise(filter)
                    return
                }
    ...
    func getFiltersIndex(_ message: EmptyValue, _ promise: @escaping (ProtoSchema.FiltersIndex) -> Void) {
        Task {
