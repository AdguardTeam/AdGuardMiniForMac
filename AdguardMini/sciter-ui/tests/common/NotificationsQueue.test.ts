...
test('notify() with timeout:false disables auto-close', async () => {
    const queue = new NotificationsQueue();

    const uuid = queue.notify(infoNotifyProps({ timeout: false }));
    assert.equal(queue.queueLength, 1);

    // Wait a bit — no auto-close should have fired
...
test('notify() with numeric timeout auto-closes after specified time', async () => {
    const queue = new NotificationsQueue();

    const uuid = queue.notify(infoNotifyProps({ timeout: 10 }));
    assert.equal(queue.queueLength, 1);
